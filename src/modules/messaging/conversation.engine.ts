...
import { resolveMessagingAccountForCompany } from "./messaging-account.resolver";
import { sendWhatsAppTextMessage } from "./whatsapp.client";

const formatQuestionForWhatsApp = (question: any) => {
  ...
  if (question.options && question.options.length > 0) {
    ...
    message += question.options
      ...
      .map((option: any, index: number) => `${index + 1}. ${option.label}`)
      .join("\n");
  }
...
const parseIncomingAnswer = (question: any, messageBody: string) => {
  ...
  if (question.type === "RATING") {
    ...
    if (Number.isNaN(numberValue)) {
      throw new AppError("Réponse invalide pour une question de notation", 400);
    }

    const selectedOption =
      question.options.find((option: any) => option.value === String(numberValue)) ||
      question.options.find((option: any) => option.order === numberValue);
  ...
  if (question.type === "SINGLE_CHOICE") {
    ...
    if (!selectedOption) {
      throw new AppError("Option invalide", 400);
    }
  ...
  if (question.type === "MULTIPLE_CHOICE") {
    ...
    const selectedOptions = question.options.filter((option: any) => {
      return parts.some(
        (part) =>
          option.value.toLowerCase() === part.toLowerCase() ||
    ...
    return {
      ...
      numberValue: null,
      booleanValue: null,
      values: selectedOptions.map((option: any) => option.value),
      selectedOption: selectedOptions[0] || null,
    };
  }

  throw new AppError("Type de question non supporté", 400);
};
...
export const sendFirstQuestionToRecipient = async (recipientId: string) => {
  ...
  const firstQuestion = await prisma.surveyQuestion.findFirst({
  ...
  if (!firstQuestion) {
    throw new AppError("Aucune question définie pour cette campagne", 400);
  }

  const messagingAccount = await resolveMessagingAccountForCompany(
    recipient.campaign.companyId
  );
...
export const handleIncomingWhatsAppMessage = async ({
  ...
  const messagingAccount = await resolveMessagingAccountForCompany(contact.companyId);
  ...
  const currentQuestion = await prisma.surveyQuestion.findFirst({
    where: {
      campaignId: session.campaignId,
      id: {
  ...
  await prisma.surveyAnswer.upsert({
    ...
    update: {
      ...
      answerType: parsed.answerType as any,
